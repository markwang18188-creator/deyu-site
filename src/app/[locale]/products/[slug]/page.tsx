...
import { products, getLocalizedProduct, categoryLabels, productVideoEmbed, youtubeEmbedUrl } from '@/data/products';
import type { MediaVideo } from '@/components/product/ProductMediaSwitcher';
...
export default async function ProductDetailPage({
  ...
  // Aggregate videos across the primary product + variants for the media switcher.
  // The primary product's youtube comes first (default tab); each variant with a
  // youtubeId adds an extra thumbnail in the strip.
  const switcherVideos: MediaVideo[] = [];
  const primaryEmbed = productVideoEmbed(product);
  const primaryId = product.youtubeId ?? product.youtubeShortsId ?? null;
  if (primaryEmbed && primaryId) {
    switcherVideos.push({
      embedUrl: primaryEmbed,
      youtubeId: primaryId,
      label: `${product.model} demonstration video`,
    });
  }
  ...
  return (
    <>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            {/* Product main media — image + N video thumbnails in the left rail */}
            <ProductMediaSwitcher
              image={product.mainImage}
              alt={`${product.model} - ${product.name}`}
              videos={switcherVideos}
      ...
      {/* Variants — closely-related machine variants surfaced on this page */}
        ...
        <section className="py-12 bg-white border-t border-[#e2e8f0]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-8">
              ...
              <p className="text-[#64748b]">{t('variants_subtitle')}</p>
            </div>
            <div className="space-y-10">
              {product.variants.map((v) => (
                <div
                  key={v.model}
                  className="bg-[#f8fafc] rounded-xl border border-[#e2e8f0] p-6 lg:p-8"
                >
                  <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
                    <span className="inline-block text-xs font-semibold text-[#1e3a8a] bg-blue-50 px-3 py-1 rounded-full">
                      {v.model}
                    </span>
                      ...
                      <ul className="space-y-1.5">
                        {v.features.map((f) => (
                          <li key={f} className="flex items-start gap-2 text-sm text-[#334155]">
                            <svg className="w-4 h-4 text-[#1e3a8a] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                            {f}
                          </li>
                        ...
                        <table className="w-full text-xs">
                          <tbody>
                            {Object.entries(v.specifications).map(([key, value], i) => (
                              <tr key={key} className={i % 2 === 0 ? 'bg-white' : 'bg-[#f8fafc]'}>
                                <td className="px-3 py-2 font-medium text-[#334155] w-1/2 border-b border-[#e2e8f0]">
                                  {key}
                                </td>
                                <td className="px-3 py-2 text-[#64748b] border-b border-[#e2e8f0]">
                                  {value}
                                </td>
                              </tr>
                            ))}
                    ...
                    <div className="mt-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {v.applications.map((app) => (
                          <span
                            key={app}
                            className="bg-white border border-[#e2e8f0] text-[#334155] text-xs font-medium px-3 py-1.5 rounded-md"
                          >
                            {app}
                          </span>
                        ))}
